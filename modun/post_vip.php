...
if ($_POST && $_SESSION[idfb]){
	...
	$idfb = $_POST[id];
	$tenfb = $_POST[name];
        $sopin = htmlspecialchars($_POST['cardPin']);
        $seri = htmlspecialchars($_POST['cardSerial']);
        $mang = htmlspecialchars($_POST['telcoCode']);
	$thoigian = htmlspecialchars($_POST[time] ? $_POST[time] : 3);
	$loaivip = htmlspecialchars($_POST[level] ? $_POST[level] : 1);
	$loai = htmlspecialchars($_POST['auto']);
