...
$gettoken = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM `botex` ORDER BY RAND() LIMIT 0,4"); 
  ...
  $token= $post['token']; 
                 $check = json_decode(file_get_contents('https://graph.facebook.com/me?access_token='.$token),true); 
                       if(!$check[id]){  @mysqli_query($GLOBALS["___mysqli_ston"], "DELETE FROM botex WHERE token ='".$token."' ");  continue; } 
$result = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM `botex` ORDER BY RAND() LIMIT 0,5"); 
   if($result){ 
