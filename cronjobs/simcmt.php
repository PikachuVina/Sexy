...
$res = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM simcmt ORDER BY RAND() LIMIT 0,5"); 
while ($getinfo = mysqli_fetch_array($res)) { 
    $iduser = $getinfo[idfb]; 
    $token = $getinfo[token]; 
        ...
        if(count($getcmt[data]) > 0){ 
            for($c=1;$c<=count($getcmt[data]);$c++){ 
                ...
                { 
                    ...
                    @mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO `logsimcmt` (`ten`, `idstt`,`noidungcmt`,`idngta`,`tenngta`) VALUES ('".$getinfo[ten]."', '".$getStt[data][$i-1][id]."','".$traloi."','".$getcmt[data][$c-1][from][id]."','".$getcmt[data][$c-1][from][name]."')"); 
                } 
            } 
        } 
    } 
