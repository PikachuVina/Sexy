...
$res = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM simcmtfeed ORDER BY RAND() LIMIT 0,5"); 
    ...
    for($i=0;$i<count($home);$i++){     
        ...
        echo'Đã Trả Like!'; 
            ...
            if($home[data][$i]){ 
                ...
                @mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO `logsimfeed` (`ten`, `idstt`,`noidungcmt`,`idngta`,`tenngta`) VALUES ('".$getinfo[ten]."', '".$home[data][$i][id]."','".$traloi."','".$home[data][$i][from][id]."','".$home[data][$i][from][name]."')"); 
                } 
            } 
        } 
    } 
