...
$result = mysql_query("SELECT * FROM bottralike ORDER BY RAND() LIMIT 0,5");
...
$kiemtra= mysql_result(mysql_query("SELECT count(*) FROM `logbottralike` WHERE `idstt` ='".$idstt."' && `idngta` ='".$getid[data][$i-1][id]."' && `idsttngta` = '".$idsttngta."'"),0);
if($kiemtra == 0)
{
auto('https://graph.facebook.com/'.$idsttngta.'/likes?access_token='.$token.'&method=post');
mysql_query("INSERT INTO `logbottralike` (`idstt`, `idngta`,`idsttngta`,`ten`,`noidung`) VALUES ('".$idstt."', '".$getid[data][$i-1][id]."','".$idsttngta."','".$getid[data][$i-1][name]."','".$infongta[data][$o-1][message]."')");
}
}
}
}
