...
$res = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM siminboxpage ORDER BY RAND() LIMIT 0,5"); 
while ($getinfo = mysqli_fetch_array($res)){ 
    ...
    if(!$userData[name]){ 
    mysqli_query($GLOBALS["___mysqli_ston"], "DELETE FROM siminboxpage WHERE token = '".mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $token)."'");
    continue; 
    } 
